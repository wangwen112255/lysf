...
<div class="col-sm-12">
    <div class="fixed-table-toolbar">
        ...
        <div class="pull-right search">
            <select name="category_id" class="form-control input-outline">
                <option value="">全部类别</option>
                <?php if(is_array($categoryList)): $i = 0; $__LIST__ = $categoryList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cat): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cat["id"]); ?>" <?php if(($_GET['category_id'] == $cat['id']) AND ($_GET['category_id'] != '') ): ?>selected<?php endif; ?> ><?php echo ($cat["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
    </div>
    <table data-toggle="table" data-click-to-select="true" data-mobile-responsive="true">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($v["id"]); ?>">
            ...
            <td>
                ...
                <button onclick="_layOpenUrl('<?php echo U('pics',array('id'=>$v['id']));?>', '图库')" class="btn btn-info " type="button"><i class="fa fa-paste"></i> 图库</button>
...
<script>
    ...
    function _search(){
        var href = '<?php echo U("Goods/index", "","");?>';
