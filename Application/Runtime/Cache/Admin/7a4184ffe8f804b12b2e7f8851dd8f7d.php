...
<div class="row">
    ...
    <div class="col-sm-12">
        ...
        <table data-toggle="table" data-click-to-select="true" data-mobile-responsive="true">
            <thead>
            <tr>
                <th width="5%" data-field="id" data-checkbox="true"></th>
                <th  width="10%" data-field="title">名称</th>
                <th  width="10%"data-field="pic">图片</th>
                <th  width="10%"data-field="addtime">最后一次修改时间</th>
                <th  width="10%"data-field="status" data-align="center">状态</th>
                <th width="25%">操作</th>
            </tr>
            </thead>
            <tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($v["id"]); ?>">
                    <td width="5%"><input type="checkbox" name="id" value="<?php echo ($v["id"]); ?>" /></td>
                    <td><?php echo ($v["title"]); ?></td>
                    <td><img style="width:120px;height: 40px;border-radius: 5px;" src="<?php echo ($v["pic"]); ?>"/></td>
                    <td><?php echo ($v["addtime"]); ?></td>
                    <td>
                        <?php if(($v["status"]) == "1"): ?><button id="status<?php echo ($v["id"]); ?>" value="<?php echo ($v["id"]); ?>" onclick="_setStatus(<?php echo ($v["id"]); ?>,<?php echo ($v["status"]); ?>);"   type="button" class='btn btn-info'>启用</button>
                            <?php else: ?>
                            <button id="status<?php echo ($v["id"]); ?>"   onclick="_setStatus(<?php echo ($v["id"]); ?>,<?php echo ($v["status"]); ?>);"   type="button" class='btn btn-default'>禁用</button><?php endif; ?>
                    </td>
                    <td>
                        <button onclick="_layOpenUrl('<?php echo U('shows',array('id'=>$v['id']));?>', '查看')" class="btn btn-info " type="button"><i class="fa fa-paste"></i> 查看</button>
